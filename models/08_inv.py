        ...
        def inv_recv_represent(id):
            ...
            if id:
                ...
                inv_recv_row = db(table.id == id).select(table.date,
                                                         table.from_location_id,
                                                         table.organisation_id,
                                                         limitby=(0, 1)).first()
                return SPAN( gis_location_represent( inv_recv_row.from_location_id),
                ...
                if recv_record:
                    rheader = DIV( TABLE(
                                       ...
                                       TR( TH( "%s: " % T("From Location")),
                                           gis_location_represent(recv_record.from_location_id),
        ...
        def inv_send_represent(id):
            if id:
                ...
                send_row = db(table.id == id).select(table.date,
                                                     table.to_location_id,
                                                     limitby=(0, 1)).first()
                return SPAN( gis_location_represent( send_row.to_location_id),
                             " - ",
                            send_row.date)
            else:
                return NONE
                ...
                if send_record:
                    rheader = DIV( TABLE(
                                       ...
                                       TR( TH("%s: " % T("From")),
                                           ...
                                           TH("%s: " % T("To")),
                                           gis_location_represent(send_record.to_location_id),
                                          ),
                                       TR( TH("%s: " % T("Status")),
