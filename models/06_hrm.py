...
if deployment_settings.has_module(module):
    ...
    # -------------------------------------------------------------------------
    def hrm_human_resource_is_duplicate(job):
        """
            This callback will be called when importing human resource records
            it assumes that the record being imported is a duplicate,
            it will set the job method to update,
            and get the id from the person_id if it can.
        ...
        if job.id:
            return
        if job.tablename == "hrm_human_resource":
            # try and extract the person id from the right job item
            # Get the uid of the pr_person referenced record
            person_id = None
            if len(job.references) > 0:
                uuid = job.references[0]["entry"]["item_id"]
                jobitem = job.job.items[uuid]
                if jobitem.id != None:
                    # Now get the id that should have been set up from the 
                    # duplicate callback to pr_person 
                    person_id = jobitem.id
            if person_id == None:
                return
            # now with the person_id find the hrm id
    ...
    s3mgr.configure("hrm_human_resource",
                    resolve=hrm_human_resource_is_duplicate)
