...
def warehouse():
    ...
    ######################################################################
    # Experimental code for importer
    ######################################################################
    def importerPrep():
        data = dict(
            Label = "Organisation",
            Widget = S3OrganisationAutocompleteWidget(),
            Lookup = db.org_organisation.id,
            Field = db.org_organisation.name,
            Represent = organisation_represent,
            # For Stock Position only
            Stylesheet = "stock_posn.xsl")
        return data

    response.s3.importerPrep = importerPrep
    ######################################################################
    # End of experimental code for importer
    ######################################################################
...
def send_process():
    ...
    try:
        table = db.org_site
        query = (table.location_id == send_record.to_location_id) & \
                (table.deleted == False)
        to_site_id = db(query).select(table.id,
...
# =============================================================================
def inv_timeline():
    send_desc = lambda x: "Quantity: %d" % (x.quantity)

    tl = simileTimeline.SimileTimeline()
    tl.addEventSource(table=db.inv_recv, title='type', desc='comments', start='date')
    tl.addEventSource(table=db.inv_send_item, title='inv_item_id', desc=send_desc, start='send_id.date', end='send_id.delivery_date')
