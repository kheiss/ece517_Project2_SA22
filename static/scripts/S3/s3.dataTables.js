    ...
    function createSubmitBtn() {
        if (S3.dataTables.Selectable)
        	...
        	if (S3.dataTables.UsePostMethod){
        		...
        		$('<div class="actionButton"><input type="submit" class="action-btn" id="submitSelection" value="'+S3.dataTables.PostSubmitLabel+'"></div>').insertBefore(paginateBtns);
        	} else {
	            URL = S3.dataTables.SelectURL
	            	...
	            	+ selected.join(',')
	            	+ '&post';
	            $('<div class="actionButton"><a class="action-btn" id="submitSelection" href='+URL+'>Submit</a></div>').insertBefore(paginateBtns);
        }
    }
